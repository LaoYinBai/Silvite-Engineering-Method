...
- 通用 Skill 单列 `Direct Execution Before Tooling` 硬约束：小规模、一次性、低风险工作优先直接交付；自动化需有当前规模、实际复用、审计或风险收益依据。
- 保留 `ZERO-DELIVERABLE WARNING`：辅助文件持续增长而正式成果为零时，停止工具扩张并产生第一个可验收成果；高风险批量操作仍使用最小必要工具与恢复门禁。
- 增加两个压力场景，分别检查工具化偏航与高风险批量操作的例外。

### Validation

- 本轮依据维护者提供的失败观察收紧默认决策规则；没有独立 Agent RED/GREEN 运行记录，行为改进尚未验证。
